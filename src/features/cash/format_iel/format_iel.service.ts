...
@Injectable()
export class CashFormatIelService {
  constructor(
    @InjectRepository(CashFormatIel)
    private readonly repo: Repository<CashFormatIel>,
  ) {}

  async create(dto: CreateCashFormatIelDto) {
    try {
  ...
  async findOne(id: number) {
    try {
      ...
      if (!result) {
        throw new HttpException(
          { data: null, message: 'IEL format not found', statusCode: 404 },
  ...
  async update(id: number, dto: UpdateCashFormatIelDto) {
    try {
