...
@Injectable()
export class FormatCentralClientsService {
  constructor(
    @InjectRepository(FormatCentralClient)
    private readonly repo: Repository<FormatCentralClient>,
  ) {}

  async create(dto: CreateFormatCentralClientDto) {
    try {
    ...
    try {
      ...
      if (!result) {
        throw new HttpException(
          {
            data: null,
            message: 'Format Central Client not found',
            statusCode: 404,
          },
  ...
  async update(id: number, dto: UpdateFormatCentralClientDto) {
    try {
