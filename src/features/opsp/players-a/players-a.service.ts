...
@Injectable()
export class PlayersAService {
  constructor(
    @InjectRepository(PlayerA)
    private readonly repo: Repository<PlayerA>,
  ) {}

  async create(dto: CreatePlayerADto) {
    try {
    ...
    try {
      ...
      if (!result) {
        throw new HttpException(
          {
            data: null,
            message: 'Player A record not found',
            statusCode: 404,
          },
  ...
  async update(id: number, dto: UpdatePlayerADto) {
    try {
