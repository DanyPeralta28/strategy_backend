...
@Injectable()
export class FormatCompetenciesService {
  constructor(
    @InjectRepository(FormatCompetency)
    private readonly repo: Repository<FormatCompetency>,
  ) {}

  async create(dto: CreateFormatCompetencyDto) {
    try {
    ...
    try {
      ...
      if (!result) {
        throw new HttpException(
          {
            data: null,
            message: 'Format Competency not found',
            statusCode: 404,
          },
  ...
  async update(id: number, dto: UpdateFormatCompetencyDto) {
    try {
