...
@Injectable()
export class FollowUpStartWeeksService {
  constructor(
    @InjectRepository(FollowUpStartWeeks)
    private readonly repo: Repository<FollowUpStartWeeks>,
  ) {}

  async create(dto: CreateFollowUpStartWeeksDto) {
    try {
    ...
    try {
      ...
      if (!result) {
        throw new HttpException(
          {
            data: null,
            message: 'Start Week record not found',
            statusCode: 404,
          },
  ...
  async update(id: number, dto: UpdateFollowUpStartWeeksDto) {
    try {
