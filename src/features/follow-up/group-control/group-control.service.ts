...
export class GroupControlService {
  constructor(
    @InjectRepository(GroupControl)
    private readonly repo: Repository<GroupControl>,
  ) {}

  async create(dto: CreateGroupControlDto) {
    try {
    ...
    try {
      ...
      if (!result) {
        throw new HttpException(
          {
            data: null,
            message: 'Group Control record not found',
            statusCode: 404,
          },
  ...
  async update(id: number, dto: UpdateGroupControlDto) {
    try {
